...
function PatientController($scope, Patient) {
	...
	$scope.currentAuthor = new Patient();
    $scope.patients = Patient.query();

    $scope.cancel = function () {
        $scope.currentPatient = new Patient();
    };

    $scope.save = function () {
    	if (!$scope.patientForm.$invalid) {
	        var isNew = $scope.currentPatient.id == null;
	        if (isNew) {
	            $scope.currentPatient = Patient.save($scope.currentPatient);
	            $scope.patients.push($scope.currentPatient);
	            
	        } else {
	            $scope.currentPatient = Patient.update($scope.currentPatient);
	        }
	        $scope.cancel();
    ...
    $scope.save = function () {
    	if (!$scope.patientForm.$invalid) {
	        ...
	        if (isNew) {
	            ...
	            $scope.currentPatient = Author.update($scope.currentPatient);
	        }
	        $scope.cancel();
	        $scope.patientForm.$setPristine();
    	}
