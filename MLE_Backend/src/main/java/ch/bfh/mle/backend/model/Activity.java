...
@NamedQueries({
    ...
    @NamedQuery(
            ...
            query="SELECT SUM(a.number * m.duration) FROM TreatmentCase as t JOIN t.activities AS a JOIN a.supplier AS s JOIN a.tarmedActivity AS m WHERE t.treatmentNumber = :treatmentNumber AND s.employeeID = :employeeId"),
    @NamedQuery(
            name="Activity.CumulatedTimesByTreatmentCase", 
            query="SELECT SUM(a.number * m.duration) FROM TreatmentCase as t JOIN t.activities AS a JOIN a.tarmedActivity AS m WHERE t.treatmentNumber = :treatmentNumber")
})
